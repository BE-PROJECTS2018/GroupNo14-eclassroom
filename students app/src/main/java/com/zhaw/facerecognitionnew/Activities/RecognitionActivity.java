...
public class RecognitionActivity extends Activity implements CameraBridgeViewBase.CvCameraViewListener2 {
    ...
    private String student_name = "Nikhil";
    private String confirm_student_name = "";
    static {
    ...
    public Mat onCameraFrame(CameraBridgeViewBase.CvCameraViewFrame inputFrame) {
        ...
        if(images == null || images.size() == 0 || faces == null || faces.length == 0 || ! (images.size() == faces.length)){
            ...
            confirm_student_name = rec.recognize(images.get(0), "") + "";
            for(int i = 0; i<faces.length; i++){
                MatOperation.drawRectangleAndLabelOnPreview(imgRgba, faces[i], rec.recognize(images.get(i), ""), front_camera);
            }
            if(!confirm_student_name.equals("")){
                Intent in = new Intent(RecognitionActivity.this,AttendanceMarked.class);
                if(confirm_student_name.equals(student_name)){
                    in.putExtra("attendance","marked");

                }else{
                    in.putExtra("attendance","notmarked");
                }
                startActivity(in);
            }
            return imgRgba;
